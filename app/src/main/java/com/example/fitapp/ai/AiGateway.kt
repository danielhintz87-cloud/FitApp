...
object AiGateway {
    ...
    suspend fun generateRecipes(prompt: String, provider: Provider = Provider.OPENAI): List<UiRecipe> = withContext(Dispatchers.IO) {
        ...
        val raw = when (provider) {
            Provider.OPENAI -> {
                val content = JSONArray().apply {
                    put(JSONObject(mapOf("type" to "text", "text" to "Schätze Kalorien des Essens auf dem Foto. Antworte kurz: 'Schätzung: <ZAHL> kcal'. Nenne zusätzlich eine kurze Begründung und Unsicherheit.")))
            ...
            Provider.GEMINI -> geminiVision(base64, "Schätze Kalorien des Essens. Formatiere: Schätzung: <ZAHL> kcal. Begründung + Unsicherheit.")
        }
    ...
    private fun openAiChat(system: String, user: String): String {
        ...
        val req = Request.Builder()
            ...
            .post(create("application/json".toMediaType(), body))
            .build()
