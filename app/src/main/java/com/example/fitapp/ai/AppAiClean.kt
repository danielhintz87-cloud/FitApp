...
object AppAiClean {
    ...
    fun getProviderStatus(context: Context): String {
        ...
        return buildString {
            ...
            appendLine(
                "- Gemini: ${if (geminiKey.isNotBlank()) {
                    "✓ Configured (${geminiKey.take(
                        10,
                    )}...)"
                } else {
                    "✗ Not configured"
            ...
            appendLine(
                "- Perplexity: ${if (perplexityKey.isNotBlank()) {
                    "✓ Configured (${perplexityKey.take(
                        10,
                    )}...)"
                } else {
                    "✗ Not configured"
            ...
            appendLine(
                "  - Gemini: ${totalUsage.geminiStats.requests} requests, ${totalUsage.geminiStats.tokens} tokens, $${String.format(
                    Locale.GERMANY,
                    "%.3f",
                    totalUsage.geminiStats.estimatedCost,
                )}",
            )
            appendLine(
                "  - Perplexity: ${totalUsage.perplexityStats.requests} requests, ${totalUsage.perplexityStats.tokens} tokens, $${String.format(
                    Locale.GERMANY,
                    "%.3f",
                    totalUsage.perplexityStats.estimatedCost,
                )}",
            )
        }
    }
    ...
    suspend fun plan(
        context: Context,
        req: PlanRequest,
    ) = planWithOptimalProvider(context, req)

    suspend fun recipes(
        context: Context,
        req: RecipeRequest,
    ) = recipesWithOptimalProvider(context, req)

    suspend fun calories(
        context: Context,
        bitmap: Bitmap,
        note: String = "",
    ) = caloriesWithOptimalProvider(
        context,
        bitmap,
        note,
    )
