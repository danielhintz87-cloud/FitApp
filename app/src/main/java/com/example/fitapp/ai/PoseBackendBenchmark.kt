...
object PoseBackendBenchmark {
    ...
    fun toCsv(results: List<Result>): String =
        buildString {
    ...
    suspend fun run(
        context: Context,
        variants: List<AdvancedMLModels.PoseModelType> =
            listOf(
        ...
        withContext(Dispatchers.Default) {
            ...
            for (variant in variants) {
                ...
                AdvancedMLModels.forceOnnxBackend(
                    if (useOnnx && variant == AdvancedMLModels.PoseModelType.MOVENET_LIGHTNING) true else false,
                )
