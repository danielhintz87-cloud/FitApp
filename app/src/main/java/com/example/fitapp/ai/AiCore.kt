...
class AiCore(private val context: Context, private val logDao: AiLogDao) {
    ...
    suspend fun estimateCaloriesFromPhoto(
        ...
        return withContext(Dispatchers.IO) {
            ...
            r.onSuccess { caloriesEstimate ->
                ...
                logDao.insert(
                    AiLog.success("vision_calories", provider.name, "$prompt $note", caloriesEstimate.toString(), took),
                )
            }.onFailure {
                logDao.insert(
                    AiLog.error("vision_calories", provider.name, "$prompt $note", it.message ?: "unknown", took),
                )
            }
            r
        }
    ...
    private suspend fun geminiChat(prompt: String): Result<String> =
        runCatching {
            withRetry {
                val apiKey = ApiKeys.getGeminiKey(context)
                if (apiKey.isBlank()) {
                    throw IllegalStateException(
                        "Gemini API-Schlüssel nicht konfiguriert. Bitte unter Einstellungen → API-Schlüssel eingeben.",
                    )
                }
    ...
    private suspend fun geminiVision(
        prompt: String,
        bitmap: Bitmap,
    ): Result<CaloriesEstimate> =
        runCatching {
    ...
    private suspend fun perplexityChat(prompt: String): Result<String> =
        runCatching {
            withRetry {
