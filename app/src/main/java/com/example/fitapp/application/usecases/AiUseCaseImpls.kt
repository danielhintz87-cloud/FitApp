...
class GenerateTrainingPlanUseCaseImpl(
    ...
    override suspend fun execute(request: PlanRequest): Result<String> {
        ...
        val prompt = buildString {
            append("Erstelle einen wissenschaftlich fundierten **${request.weeks}-Wochen-Trainingsplan** in Markdown. ")
            append("Ziel: ${request.goal}. Trainingsfrequenz: ${request.sessionsPerWeek} Einheiten/Woche, ${request.minutesPerSession} Min/Einheit. ")
        ...
        return repository.generateText(aiRequest).recoverCatching { error ->
            ...
            if (fallbackProvider != null && repository.isProviderAvailable(fallbackProvider)) {
                val fallbackRequest = aiRequest.copy(
...
class GenerateRecipesUseCaseImpl(
    ...
    override suspend fun execute(request: RecipeRequest): Result<String> {
        ...
        val prompt = buildString {
            append("Erstelle ${request.count} **einzelne, klar getrennte Rezepte** als strukturierte Markdown-Liste. ")
...
class EstimateCaloriesUseCaseImpl(
    ...
    override suspend fun execute(bitmap: Bitmap, note: String): Result<CaloriesEstimate> {
        val prompt = buildString {
            ...
            append("2. Schätze Portionsgrößen anhand von Referenzobjekten (Teller ≈ 25cm, Gabel ≈ 20cm, Hand ≈ 18cm)\n")
...
class ParseShoppingListUseCaseImpl(
    ...
    override suspend fun execute(spokenText: String): Result<String> {
        ...
        val prompt = buildString {
            append("Analysiere folgenden gesprochenen Text und extrahiere einzelne Einkaufsliste-Items: '$spokenText'\n\n")
