...
class DigitalCoachManager(
     ...
     */
    suspend fun getRecommendedCoachingTriggers(): List<CoachingTrigger> =
        withContext(Dispatchers.IO) {
            try {
    ...
    private suspend fun generateDailyCheckInMessage(
        stats: UserStats,
        ...
        when {
            ...
            stats.hasCompletedWorkoutToday -> {
                ...
                if (!stats.hasLoggedNutritionToday) {
                    messages.add(
                        "Vergiss nicht, deine Mahlzeiten zu tracken, um das Maximum aus deinem Training herauszuholen! 🍎",
                    )
                }
            }
            activity.daysSinceLastWorkout >= 3 -> {
                messages.add(
                    "Hey! Du warst ${activity.daysSinceLastWorkout} Tage nicht aktiv. Zeit für ein Comeback! 🎯",
                )
    ...
    private suspend fun generatePostWorkoutMessage(
        stats: UserStats,
        activity: RecentActivity,
    ): CoachingMessage {
        val congratsMessages =
            listOf(
    ...
    private suspend fun generateWorkoutReminderMessage(
        stats: UserStats,
        activity: RecentActivity,
    ): CoachingMessage {
        val timeBasedMessages =
    ...
    private suspend fun generateGoalProgressMessage(
        stats: UserStats,
        activity: RecentActivity,
    ): CoachingMessage {
        val progressPercent =
    ...
    private suspend fun generateStreakMotivationMessage(
        stats: UserStats,
        activity: RecentActivity,
    ): CoachingMessage {
        return when {
    ...
    private suspend fun generateNutritionTipMessage(
        stats: UserStats,
        activity: RecentActivity,
    ): CoachingMessage {
        val tips =
    ...
    private suspend fun generateRecoveryReminderMessage(
        stats: UserStats,
        activity: RecentActivity,
    ): CoachingMessage {
        val recoveryTips =
