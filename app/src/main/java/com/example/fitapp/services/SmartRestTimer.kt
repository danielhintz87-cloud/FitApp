...
class SmartRestTimer(private val context: Context) {
    ...
    suspend fun startAdaptiveRest(
        ...
        val restSeconds = calculateAdaptiveRestTime(
            intensity = intensity,
            heartRate = heartRate,
     ...
     */
    private suspend fun startTimer(seconds: Int, suggestion: RestSuggestion) {
        timerJob?.cancel()
    ...
    private fun generateRestSuggestion(
        ...
        val nextSetRecommendation = when {
            intensity > 0.8f -> NextSetRecommendation(
                suggestion = "Maintain current weight, focus on form",
                weightAdjustment = 0f,
                repAdjustment = 0,
                reason = "High intensity set - prioritize recovery and form"
            )
            intensity > 0.6f -> NextSetRecommendation(
