...
class PersonalStreakManager(
    ...
    companion object {
        ...
        private fun LocalDate.toEpochSecond(): Long = this.atStartOfDay(ZoneId.systemDefault()).toEpochSecond()

        private fun Long.toLocalDate(): LocalDate =
            Instant.ofEpochSecond(this).atZone(ZoneId.systemDefault()).toLocalDate()
    }

    fun getAllStreaks(): Flow<List<PersonalStreakEntity>> = repository.allStreaksFlow()

    fun getActiveStreaks(): Flow<List<PersonalStreakEntity>> = repository.activeStreaksFlow()
    ...
    suspend fun trackWorkoutCompletion(date: LocalDate = LocalDate.now()) {
        ...
        val workoutStreaks =
            repository.streaksByCategoryFlow(CATEGORY_WORKOUT).first()
                .filter { it.isActive }
    ...
    suspend fun trackNutritionLogging(date: LocalDate = LocalDate.now()) {
        val dateTimestamp = date.toEpochSecond()
        val nutritionStreaks =
            repository.streaksByCategoryFlow(CATEGORY_NUTRITION).first()
                .filter { it.isActive }
    ...
    suspend fun trackWeightLogging(date: LocalDate = LocalDate.now()) {
        val dateTimestamp = date.toEpochSecond()
        val weightStreaks =
            repository.streaksByCategoryFlow(CATEGORY_WEIGHT).first()
                .filter { it.isActive }
    ...
    private suspend fun updateHabitStreak(
        streak: PersonalStreakEntity,
        todayTimestamp: Long,
    ) {
    ...
    private suspend fun updateWeightTrackingStreak(
        streak: PersonalStreakEntity,
        todayTimestamp: Long,
    ) {
        val todayIso = todayTimestamp.toLocalDate().format(DateTimeFormatter.ISO_LOCAL_DATE)
    ...
    private suspend fun incrementStreak(
        streak: PersonalStreakEntity,
        timestamp: Long,
    ) {
        ...
        val newCurrentStreak =
            if (lastActivity == null || lastActivity.isBefore(currentDate)) {
    ...
    private suspend fun checkForStreakBreak(
        streak: PersonalStreakEntity,
        todayTimestamp: Long,
    ) {
        if (streak.lastActivityTimestamp != null) {
