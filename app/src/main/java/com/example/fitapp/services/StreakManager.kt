...
class StreakManager(
     ...
     */
    suspend fun onMealLogged(date: LocalDate = LocalDate.now()) =
     ...
     */
    suspend fun getCurrentMealLoggingStreak(): PersonalStreakEntity? =
        withContext(Dispatchers.IO) {
            try {
     ...
     */
    suspend fun initializeMealLoggingAchievements() =
        withContext(Dispatchers.IO) {
            try {
     ...
     */
    private fun updateStreakForDate(
