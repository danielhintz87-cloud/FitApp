...
class BodyweightProgressionManager(private val context: Context) {
    ...
    suspend fun calculateBodyweightProgression(
        ...
        try {
            ...
            when {
                !canProgress -> ProgressionRecommendation.maintain(
                    0f, currentReps ?: 0, 1, 
                    "Fokus auf Bewegungsqualität und Form"
                )
                
                isEasyEffort -> calculateAggressiveBodyweightProgression(
                    currentReps, currentTime, currentDifficulty, exerciseCategory
                )
                
                else -> calculateConservativeBodyweightProgression(
                    currentReps, currentTime, currentDifficulty, exerciseCategory
                )
            }
        } catch (e: Exception) {
    ...
    fun getDefaultBodyweightExercises(): List<BodyweightExercise> {
        return listOf(
            ...
            BodyweightExercise(
                ...
                progressionOptions = listOf(
                    BodyweightProgression(ProgressionType.REP_INCREASE, "2-3 Wiederholungen", "Mehr Wiederholungen"),
                    BodyweightProgression(ProgressionType.DIFFICULTY_INCREASE, "Nächste Schwierigkeit", "Diamant-Liegestütze", 1)
                )
            ),
