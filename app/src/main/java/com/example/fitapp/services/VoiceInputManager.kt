...
class VoiceInputManager(private val context: Context) {
     ...
     */
    fun startListeningForShoppingItems(): Flow<VoiceInputResult> =
        callbackFlow {
    ...
    private fun parseIndividualItem(text: String): VoiceShoppingItem? {
        ...
        val quantityPatterns =
            listOf(
                Regex(
                    """(\d+(?:[.,]\d+)?)\s*(kg|kilogramm|kilo|gramm|g|liter|l|ml|stück|stk|packung|pack|dose|dosen|flasche|flaschen)\s*(.+)""",
                ),
                Regex("""(\d+(?:[.,]\d+)?)\s*(.+)"""),
