...
class VideoManager(private val context: Context) {
     ...
     */
    suspend fun preloadWorkoutVideos(exerciseIds: List<String>) = withContext(Dispatchers.IO) {
    ...
    ): VideoResource? = withContext(Dispatchers.IO) {
        try {
     ...
     */
    fun downloadVideoInBackground(exerciseId: String): Flow<DownloadProgress> = flow {
        try {
     ...
     */
    suspend fun manageCacheSize() = withContext(Dispatchers.IO) {
        try {
    ...
    private fun getBuiltInExerciseVideo(exerciseId: String, quality: VideoQuality): VideoResource {
