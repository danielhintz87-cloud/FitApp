...
class SocialChallengeManager(
    ...
    fun getAllChallenges(): Flow<List<SocialChallengeEntity>> =
        repository.allSocialChallengesFlow()
    
    fun getActiveChallenges(): Flow<List<SocialChallengeEntity>> =
        repository.challengesByStatusFlow(STATUS_ACTIVE)
    
    fun getChallengesByCategory(category: String): Flow<List<SocialChallengeEntity>> =
        repository.challengesByCategoryFlow(category)
    
    fun getOfficialChallenges(): Flow<List<SocialChallengeEntity>> =
        repository.officialChallengesFlow()
     ...
     */
    suspend fun joinChallenge(challengeId: Long, userId: String, userName: String? = null): Boolean {
        ...
        val participation = ChallengeParticipationEntity(
     ...
     */
    suspend fun leaveChallenge(challengeId: Long, userId: String) {
    ...
    suspend fun logProgress(
        ...
        // Insert progress log
        val progressLog = ChallengeProgressLogEntity(
     ...
     */
    private suspend fun completeChallenge(challengeId: Long, userId: String) {
        val participation = repository.getUserParticipation(challengeId, userId) ?: return
        val challenge = repository.getSocialChallenge(challengeId) ?: return
     ...
     */
    private suspend fun awardChallengeBadge(userId: String, challenge: SocialChallengeEntity, reward: String) {
        val badge = SocialBadgeEntity(
            title = reward,
            description = "Earned by completing ${challenge.title}",
            category = "challenge",
            badgeType = when (challenge.difficulty) {
            ...
            rarity = when (challenge.difficulty) {
    ...
    suspend fun trackWorkoutForChallenges(userId: String) {
        val activeParticipations = repository.participationsByUserFlow(userId).first()
     ...
     */
    suspend fun trackNutritionForChallenges(userId: String, calories: Int) {
        val activeParticipations = repository.participationsByUserFlow(userId).first()
            .filter { it.status == PARTICIPATION_ACTIVE }
