...
class ShoppingAutoCompleteEngine {
    ...
    fun getSuggestions(
        ...
        val exactMatches = productDatabase.filter { 
        ...
        if (suggestions.size < MAX_SUGGESTIONS) {
            val fuzzyMatches = productDatabase.filter { product ->
        ...
        if (suggestions.size < MAX_SUGGESTIONS) {
            val recentMatches = recentItems.filter { item ->
        ...
        if (suggestions.size < MAX_SUGGESTIONS && categoryHint != null) {
            val categoryMatches = productDatabase.filter { product ->
        ...
        return suggestions
            .sortedWith(compareByDescending<AutoCompleteSuggestion> { it.confidence }
    ...
    private fun getFrequentSuggestions(
        ...
        // Most frequent items
        if (suggestions.size < MAX_SUGGESTIONS) {
            val frequentItems = usageFrequency.entries
     ...
     */
    private fun isFuzzyMatch(query: String, product: String): Boolean {
     ...
     */
    private fun calculateFuzzyConfidence(query: String, product: String): Float {
     ...
     */
    fun getPopularItemsInCategory(category: String, limit: Int = 5): List<String> {
