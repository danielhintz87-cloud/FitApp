...
class SimilarRecipesEngine(
    ...
    private suspend fun generateAIVariations(
        ...
        for (variationType in variationTypes) {
            try {
                ...
                val request =
                    RecipeRequest(
                        preferences = "Similar to ${originalRecipe.title}. ${extractCuisineStyle(originalRecipe) ?: "International cuisine"}. Key ingredients: ${extractKeyIngredients(
                            originalRecipe,
                        ).take(5).joinToString(", ")}",
    ...
    private suspend fun generateSpecificVariation(
        ...
        try {
            ...
            val request =
                RecipeRequest(
                    preferences = "Recipe based on ${originalRecipe.title}. Cuisine: ${feedback.preferredCuisine ?: extractCuisineStyle(originalRecipe)}. Ingredients: ${extractKeyIngredients(
                        originalRecipe,
                    ).filter { it !in feedback.dislikedIngredients }.take(5).joinToString(", ")}",
                    diet =
                        when {
    ...
    private fun calculateSimilarity(
        recipe1: SavedRecipeEntity,
        recipe2: SavedRecipeEntity,
    ): SimilarityResult {
        var score = 0f
    ...
    private fun createVariationPrompt(
    ...
    private fun adjustCaloriesForVariation(
    ...
    private fun adjustPrepTimeForVariation(
        originalPrepTime: Int?,
        variationType: VariationType,
    ): Int? {
        return when (variationType) {
    ...
    private fun parseGeneratedRecipe(
        ...
        val title =
            lines.find { it.startsWith("##") }?.removePrefix("## ")?.trim()
    ...
    private fun generatePreferenceReasons(
        recipe: SavedRecipeEntity,
        preferences: DietaryPreferences,
    ): List<String> {
        val reasons = mutableListOf<String>()
