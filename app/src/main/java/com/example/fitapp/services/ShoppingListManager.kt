...
class ShoppingListManager(
     ...
     */
    suspend fun markIngredientAsPurchased(
        itemId: String,
    ...
    fun mergeIngredients(
        ...
        val (mergedQuantity, mergedUnit) =
            if (areUnitsCompatible(item1.unit, item2.unit)) {
        ...
        return item1.copy(
            ...
            unit = mergedUnit,
            addedFrom =
                if (item1.addedFrom == item2.addedFrom) {
                    item1.addedFrom
                } else {
    ...
    private suspend fun updateShoppingItem(item: ShoppingListItem) {
        val existingDbItem =
            database.shoppingDao().itemsFlow().first()
    ...
    private fun isSimilarIngredient(
        name1: String,
        name2: String,
    ): Boolean {
    ...
    private fun adjustQuantityForServings(
        quantityStr: String,
        servings: Int,
    ): Double {
