...
class MealPlanningManager(
    ...
    suspend fun optimizePrepTimeScheduling(
        ...
        withContext(Dispatchers.IO) {
            try {
                val dailyPrepTimes =
                    weeklyPlan.weekPlan.mapValues { (_, dailyPlan) ->
    ...
    private fun calculateMacroBalance(
        meals: List<PlannedMeal>,
        target: MacroCalculationResult,
    ): MacroBalance {
    ...
    private fun generateMealPrepTips(
        weeklyPlan: Map<DayOfWeek, DailyMealPlan>,
    ...
    private fun adjustCookingTimeForServings(
        originalTime: Int,
        scalingFactor: Float,
    ): Int {
    ...
    private fun suggestMealPrepDays(
...
data class Recipe(
    ...
    fun containsAnimalProducts(): Boolean =
        containsMeat() || containsDairy() ||
            ingredients.any { it.name.lowercase().contains("egg") }

    fun containsGluten(): Boolean =
        ingredients.any {
    ...
    fun isPaleoCompliant(): Boolean =
        !containsGluten() && !containsDairy() &&
