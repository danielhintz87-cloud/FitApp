...
class CookingModeManager(
    ...
    fun navigateToNextStep(): CookingStep? {
        ...
        if (nextIndex < currentFlow.steps.size) {
            ...
            val updatedSteps = currentFlow.steps.mapIndexed { index, step ->
     ...
     */
    suspend fun startStepTimer(stepIndex: Int, duration: Int, name: String = "Timer") {
        ...
        val timerEntity = CookingTimerEntity(
    ...
    suspend fun markStepComplete() {
        ...
        currentTimers[currentStepIndex]?.let { timer ->
            currentTimers[currentStepIndex] = timer.copy(
                isActive = false,
                isCompleted = true
            )
            _stepTimers.value = currentTimers
    ...
    suspend fun pauseCooking() {
        ...
        val currentTimers = _stepTimers.value.mapValues { (_, timer) ->
            if (timer.isActive) {
        ...
        for (line in lines) {
            ...
            when {
                ...
                trimmedLine.matches(Regex("^(\\d+)\\s*\\.\\s*.*")) -> {
                    ...
                    // Start new step
                    currentInstruction = trimmedLine.replace(Regex("^#{1,3}\\s*[Ss]chritt\\s+\\d+:?\\s*"), "")
                        .replace(Regex("^\\d+\\.\\s*"), "")
    ...
    private fun createCookingStep(
        ...
        val duration = timerMatch?.let { match ->
    ...
    private fun parseIngredient(text: String): Ingredient? {
        ...
        return if (match != null) {
            ...
            val isOptional = text.contains("optional", ignoreCase = true) || 
    ...
    private fun startTimerCountdown(timerId: String, initialDuration: Int) {
        scope.launch {
            ...
            currentTimers.values.find { timer -> 
                ...
                val stepIndex = timer.stepIndex
                currentTimers[stepIndex] = timer.copy(
                    remainingTime = 0,
                    isActive = false,
                    isCompleted = true
                )
