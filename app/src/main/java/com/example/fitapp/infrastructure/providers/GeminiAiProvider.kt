...
class GeminiAiProvider
    ...
    constructor(
         ...
         */
        suspend fun generateTextWithTaskType(
            prompt: String,
            taskType: TaskType,
        ): Result<String> =
            safeApiCall(context, "Gemini Text Generation") {
                ...
                val model =
                    when (modelSelection.geminiModel) {
                ...
                withContext(ioDispatcher) {
                    ...
                    if (!response.isSuccessful) {
                        ...
                        android.util.Log.w(
                            logTag,
                            "http_error code=${response.code()} aiErr=${aiErr.code} bodySnippet=${bodyStr.take(120)}",
                        )
        ...
        override suspend fun analyzeImage(
            prompt: String,
            bitmap: Bitmap,
        ): Result<CaloriesEstimate> = analyzeImageWithTaskType(prompt, bitmap, TaskType.CALORIE_ESTIMATION)
         ...
         */
        suspend fun analyzeImageWithTaskType(
            prompt: String,
            bitmap: Bitmap,
            taskType: TaskType,
