        ...
        return when {
            ...
            taskType.requiresCurrentInfo() -> {
                if (currentPerplexityUsage < MONTHLY_PERPLEXITY_CAPACITY) {
                    ...
                    selectGeminiForComplexTask(taskType, currentFlashUsage, currentFlashLiteUsage, 
        ...
        return BudgetRecommendation(
            priorityActions = buildList {
                if (status.flashRemaining > 50) {
                    add("🎯 Premium Features nutzen: ${status.flashRemaining} Flash-Requests für komplexe Trainingspläne")
                }
...
// Helper Extensions
private fun TaskType.isComplex(): Boolean = when (this) {
    TaskType.TRAINING_PLAN,
    TaskType.COMPLEX_PLAN_ANALYSIS,
    TaskType.PROGRESS_ANALYSIS,
    TaskType.RECIPE_WITH_IMAGE_GEN -> true
    else -> false
}

private fun TaskType.requiresCurrentInfo(): Boolean = when (this) {
    TaskType.RESEARCH_TRENDS,
    TaskType.SUPPLEMENT_RESEARCH -> true
    else -> false
}
