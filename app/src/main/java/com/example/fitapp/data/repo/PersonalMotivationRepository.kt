...
class PersonalMotivationRepository @Inject constructor(private val db: AppDatabase) {
    ...
    suspend fun markAchievementCompleted(
        id: Long,
        completed: Boolean,
        completedAt: Long?,
    ...
    suspend fun getAchievement(id: Long): PersonalAchievementEntity? = db.personalAchievementDao().getAchievement(id)

    // Streak methods
    fun allStreaksFlow(): Flow<List<PersonalStreakEntity>> = db.personalStreakDao().allStreaksFlow()

    fun activeStreaksFlow(): Flow<List<PersonalStreakEntity>> = db.personalStreakDao().activeStreaksFlow()
    ...
    suspend fun insertStreak(streak: PersonalStreakEntity): Long = db.personalStreakDao().insert(streak)

    suspend fun updateStreak(streak: PersonalStreakEntity) = db.personalStreakDao().update(streak)

    suspend fun updateStreakCounts(
        id: Long,
        currentStreak: Int,
        longestStreak: Int,
    ...
    suspend fun setStreakActive(
        id: Long,
        active: Boolean,
    ) = db.personalStreakDao().setActive(id, active)

    suspend fun getStreak(id: Long): PersonalStreakEntity? = db.personalStreakDao().getStreak(id)

    // Personal records methods
    fun allRecordsFlow(): Flow<List<PersonalRecordEntity>> = db.personalRecordDao().allRecordsFlow()
    ...
    suspend fun insertRecord(record: PersonalRecordEntity): Long = db.personalRecordDao().insert(record)

    suspend fun getBestRecord(
        exerciseName: String,
        recordType: String,
    ): PersonalRecordEntity? = db.personalRecordDao().getBestRecord(exerciseName, recordType)

    suspend fun getExerciseNames(): List<String> = db.personalRecordDao().getExerciseNames()

    // Progress milestones methods
    fun allMilestonesFlow(): Flow<List<ProgressMilestoneEntity>> = db.progressMilestoneDao().allMilestonesFlow()
    ...
    suspend fun insertMilestone(milestone: ProgressMilestoneEntity): Long = db.progressMilestoneDao().insert(milestone)

    suspend fun updateMilestone(milestone: ProgressMilestoneEntity) = db.progressMilestoneDao().update(milestone)
    ...
    suspend fun markMilestoneCompleted(
        id: Long,
        completed: Boolean,
        completedAt: Long?,
    ) = db.progressMilestoneDao().markAsCompleted(id, completed, completedAt)

    suspend fun getMilestone(id: Long): ProgressMilestoneEntity? = db.progressMilestoneDao().getMilestone(id)

    // Today workout methods for streak tracking
    suspend fun getTodayWorkout(dateIso: String): TodayWorkoutEntity? = db.todayWorkoutDao().getByDate(dateIso)

    suspend fun getWorkoutsBetween(
        fromIso: String,
        toIso: String,
    ): List<TodayWorkoutEntity> = db.todayWorkoutDao().getBetween(fromIso, toIso)

    // Intake entries for nutrition streak tracking
    suspend fun getTotalIntakeForDay(epochSec: Long): Int = db.intakeDao().totalForDay(epochSec)

    // Weight entries for weight tracking streaks
    suspend fun hasWeightEntryForDate(dateIso: String): Int = db.weightDao().hasEntryForDate(dateIso)

    // Social Challenge methods
    fun allSocialChallengesFlow(): Flow<List<SocialChallengeEntity>> = db.socialChallengeDao().allChallengesFlow()
    ...
    fun officialChallengesFlow(): Flow<List<SocialChallengeEntity>> = db.socialChallengeDao().officialChallengesFlow()

    suspend fun insertSocialChallenge(challenge: SocialChallengeEntity): Long =
        db.socialChallengeDao().insert(challenge)

    suspend fun getSocialChallenge(id: Long): SocialChallengeEntity? = db.socialChallengeDao().getChallenge(id)
    ...
    suspend fun getUserParticipation(
        challengeId: Long,
        userId: String,
    ): ChallengeParticipationEntity? = db.challengeParticipationDao().getUserParticipation(challengeId, userId)

    suspend fun updateParticipationProgress(
        id: Long,
        progress: Double,
        percentage: Double,
        date: String,
    ...
    suspend fun updateParticipationRank(
        id: Long,
        rank: Int,
    ...
    fun allSocialBadgesFlow(): Flow<List<SocialBadgeEntity>> = db.socialBadgeDao().allBadgesFlow()
    ...
    suspend fun insertSocialBadge(badge: SocialBadgeEntity): Long = db.socialBadgeDao().insert(badge)

    suspend fun getUnlockedBadgeCount(): Int = db.socialBadgeDao().getUnlockedBadgeCount()
    ...
    suspend fun insertLeaderboardEntry(entry: LeaderboardEntryEntity): Long = db.leaderboardEntryDao().insert(entry)

    suspend fun clearLeaderboard(challengeId: Long) = db.leaderboardEntryDao().clearLeaderboard(challengeId)
}
