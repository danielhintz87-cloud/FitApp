...
class UserPreferencesRepository(private val context: Context) {
    ...
    val selectedEquipment: Flow<Set<String>> = userPreferences.map { prefs ->
        prefs.selectedEquipmentList.toSet()
    }
    
    // Workout preferences
    val workoutPreferences: Flow<WorkoutPreferences> = userPreferences.map { prefs ->
        WorkoutPreferences(
    ...
    val nutritionPreferences: Flow<NutritionPreferences> = userPreferences.map { prefs ->
    ...
    // User profile
    val userProfile: Flow<UserProfile> = userPreferences.map { prefs ->
        UserProfile(
    ...
    // App preferences
    val appPreferences: Flow<AppPreferences> = userPreferences.map { prefs ->
        AppPreferences(
    ...
    // Sync timestamps
    val syncTimestamps: Flow<SyncTimestamps> = userPreferences.map { prefs ->
        SyncTimestamps(
    ...
    private fun migrateEquipment(sharedPrefs: SharedPreferences, legacyPrefs: SharedPreferences): List<String> {
