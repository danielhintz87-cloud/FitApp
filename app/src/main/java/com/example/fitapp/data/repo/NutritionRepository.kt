...
class NutritionRepository(private val db: AppDatabase, private val context: Context? = null) {
    ...
    suspend fun generateAndStoreOptimal(
        context: Context,
        prompt: String,
    ): List<UiRecipe> {
    ...
    suspend fun setFavorite(
        ...
        if (fav) {
            db.recipeDao().addFavorite(RecipeFavoriteEntity(recipeId))
        } else {
            db.recipeDao().removeFavorite(recipeId)
        }
    }
    ...
    suspend fun generateAICalorieRecommendation(): Int {
        ...
        val baselineKcal =
            when (latestPlan.goal.lowercase(java.util.Locale.ROOT)) {
    ...
    suspend fun calculateWorkoutHydration(
        ...
        val tempMultiplier =
            when (environmentTemp.lowercase()) {
                "heiß", "hot" -> 1.3f
                "warm" -> 1.1f
                else -> 1.0f
            }
    ...
    suspend fun getWeightsBetween(
        fromIso: String,
        toIso: String,
    ) = db.weightDao().getBetween(fromIso, toIso)
    ...
    suspend fun searchFoodItems(
        query: String,
        limit: Int = 20,
    ) = db.foodItemDao().searchByName(query, limit)
    ...
    suspend fun getMealEntriesForDateAndType(
        date: String,
        mealType: String,
    ) = db.mealEntryDao().getByDateAndMealType(
        date,
        mealType,
    )

    fun getMealEntriesForDateAndTypeFlow(
        date: String,
        mealType: String,
    ) = db.mealEntryDao().getByDateAndMealTypeFlow(
        date,
        mealType,
    )
    ...
    suspend fun findOrCreateFoodByBarcode(
        barcode: String,
        name: String,
        calories: Int,
        carbs: Float,
    ...
    fun calorieHistoryFlow(days: Int): Flow<List<Pair<String, Float>>> =
        flow {
    ...
    fun macroHistoryFlow(days: Int): Flow<List<Triple<String, Float, Float>>> =
        flow {
