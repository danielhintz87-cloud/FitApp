...
class WeightLossRepository(private val database: AppDatabase) {
    ...
    suspend fun getBMIHistoryByDateRange(startDate: String, endDate: String): List<BMIHistoryEntity> {
    ...
    suspend fun calculateAndSaveBMI(
        ...
        val bmiHistory = BMIHistoryEntity(
            date = date,
            height = heightCm,
            weight = weightKg,
            bmi = bmiResult.bmi,
    ...
    suspend fun createWeightLossProgram(
        ...
        val exerciseMinutes = when (activityLevel) {
    ...
    suspend fun getBehavioralCheckInsByDateRange(startTimestamp: Long, endTimestamp: Long): List<BehavioralCheckInEntity> {
