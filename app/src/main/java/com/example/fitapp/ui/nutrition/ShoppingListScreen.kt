...
fun ShoppingListScreen(repo: NutritionRepository = NutritionRepository(AppDatabase.get(LocalContext.current))) {
    ...
    Column(Modifier.fillMaxSize()) {
        Text("Einkaufsliste", style = MaterialTheme.typography.titleLarge, modifier = Modifier.padding(16.dp))
...
@Composable
private fun ShoppingRow(item: ShoppingItemEntity, onToggle: (Boolean) -> Unit, onDelete: () -> Unit) {
    ElevatedCard {
