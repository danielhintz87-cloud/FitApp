        ...
        OutlinedCard(Modifier.fillMaxWidth()) {
            Column(Modifier.padding(Spacing.md)) {
                if (plan == null) {
                    ...
                    Text("Wechsle zum Tab „Training“, um einen Grundplan zu erstellen.",
                    ...
                    if (day == null) {
                        ...
                        day.exercises.forEach {
                            Text("• ${it.name}${it.sets?.let { s -> " – ${s}×${it.reps ?: "-"}" } ?: ""}")
                        }
                        Spacer(Modifier.height(Spacing.sm))
                        Button(onClick = {
                            scope.launch {
                                val alt = runCatching {
                                    Ai.repo.suggestAlternative(plan!!.goal, plan!!.devices.firstOrNull()?.name ?: "Körpergewicht", plan!!.timeBudgetMin)
                                }.getOrElse {
                                    PlanGenerator.alternativeForToday(plan!!.goal, plan!!.devices.firstOrNull()?.name ?: "Körpergewicht", plan!!.timeBudgetMin)
                                }
