...
fun NutritionScreen() {
    ...
    var prompt by remember { mutableStateOf("10 Rezepte für Abnehmen, High-Protein, 500-700 kcal") }
            ...
            0 -> GenerateTab(
                ...
                onGenerate = {
                    ...
                    scope.launch {
                        ...
                        catch (t: Throwable) { results = emptyList() }
                        finally { generating = false }
                    }
                },
                onFav = { id, fav -> scope.launch { repo.setFavorite(id, fav) } },
                onToShopping = { id -> scope.launch { repo.addRecipeToShoppingList(id) } },
                onLog = { r -> scope.launch { repo.logIntake(r.calories ?: 0, "Rezept: ${r.title}", "RECIPE", r.id) } }
            )
