...
class ResilientMLPipeline private constructor(
    ...
    ): MLResult<Unit> = withContext(Dispatchers.IO) {
     ...
     */
    private suspend fun processFrameDegraded(bitmap: Bitmap, requestId: String): MLResult<Any> {
        return try {
    ...
    private suspend fun processFrameSync(bitmap: Bitmap): MLResult<Any> {
        return try {
            ...
            val result = advancedMLModels?.analyzePoseFromFrameOptimized(bitmap)
