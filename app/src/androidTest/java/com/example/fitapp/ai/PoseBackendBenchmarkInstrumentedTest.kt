...
class PoseBackendBenchmarkInstrumentedTest {
    @Test
    fun benchmarkLightningTfliteAndOnnx() = runBlocking {
        val ctx = InstrumentationRegistry.getInstrumentation().targetContext
        // Lightning TFLite
        AdvancedMLModels.forceOnnxBackend(false)
        val tflite = PoseBackendBenchmark.run(ctx, variants = listOf(AdvancedMLModels.PoseModelType.MOVENET_LIGHTNING), runs = 5, warmup = 1, useOnnx = false)
        // Lightning ONNX
        AdvancedMLModels.forceOnnxBackend(true)
        val onnx = PoseBackendBenchmark.run(ctx, variants = listOf(AdvancedMLModels.PoseModelType.MOVENET_LIGHTNING), runs = 5, warmup = 1, useOnnx = true)
