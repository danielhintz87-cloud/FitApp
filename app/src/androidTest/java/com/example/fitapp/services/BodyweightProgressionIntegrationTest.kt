...
class BodyweightProgressionIntegrationTest {
    ...
    @Test
    fun testBodyweightProgressionLogic() =
        runBlocking {
            // Test progression for push-ups (rep-based)
            val pushUpProgression =
                bodyweightManager.calculateBodyweightProgression(
                    exerciseId = "pushups",
                    currentReps = 10,
                    currentTime = null,
                    currentDifficulty = 2,
    ...
    @Test
    fun testProgressionFormScoreValidation() =
        runBlocking {
            // Test low form score - should maintain
            val lowFormProgression =
                bodyweightManager.calculateBodyweightProgression(
                    exerciseId = "pushups",
                    currentReps = 10,
                    currentTime = null,
    ...
    fun testExerciseCategoryClassification() {
        ...
        assertTrue(
            "Push-ups should be in PUSH category",
            pushExercises.any { it.name == "Liegestütze" },
        )
        assertTrue(
            "Burpees should be in CARDIO category",
            cardioExercises.any { it.name == "Burpees" },
        )
        assertTrue(
            "Plank should be in CORE category",
            coreExercises.any { it.name == "Plank" },
        )
        assertTrue(
            "Squats should be in SQUAT category",
