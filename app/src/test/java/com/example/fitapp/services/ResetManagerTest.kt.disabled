...
class ResetManagerTest {
    ...
    @Test
    fun `should perform workout data reset with valid token`() =
        runTest {
    ...
    @Test
    fun `should reject reset with invalid token`() =
        runTest {
    ...
    @Test
    fun `should perform nutrition data reset`() =
        runTest {
    ...
    @Test
    fun `should perform selective reset with options`() =
        runTest {
            // Given: Selective reset options
            val options =
                SelectiveResetOptions(
    ...
    @Test
    fun `should track reset progress during operation`() =
        runTest {
    ...
    @Test
    fun `should preserve user settings when requested`() =
        runTest {
    ...
    @Test
    fun `should handle reset operation errors gracefully`() =
        runTest {
    ...
    @Test
    fun `should validate different reset types correctly`() =
        runTest {
            ...
            for (resetType in resetTypes) {
                ...
                assertTrue(
                    "Result should be consistent for $resetType",
    ...
    @Test
    fun `should provide clear error messages for different failure types`() =
        runTest {
            // Test invalid token error
            val invalidTokenResult =
                resetManager.performReset(
                    ResetType.WORKOUT_DATA,
                    "INVALID",
                )
            ...
            assertTrue(
                "Should have error message for invalid token",
                invalidTokenResult.errorMessage?.isNotEmpty() == true,
            )
            assertEquals(
                "Should have correct error type",
                ResetError.INVALID_TOKEN,
                invalidTokenResult.errorType,
            )
        }

    @Test
    fun `should handle empty selective reset options`() =
        runTest {
    ...
    @Test
    fun `should support reset progress tracking`() =
        runTest {
