...
class FreeleticsAdaptiveTrainerTest {
    ...
    fun testAdaptiveExerciseModification_HasChanges_DetectsModifications() {
        // Test with changes
        val modificationWithChanges = AdaptiveExerciseModification(
            exerciseId = "push_ups",
        ...
        val modificationNoChanges = AdaptiveExerciseModification(
            exerciseId = "squats",
            reason = "No change needed"
        )
        
        assertFalse("Should not detect changes when all adjustments are default", 
    ...
    fun testAdaptiveExerciseModification_NoChange_CreatesCorrectInstance() {
        ...
        assertTrue("Should have appropriate reason", 
    ...
    fun testDifficultyAdjustment_CalculatesAdjustmentCorrectly() {
        ...
        assertTrue("Adjustment factor should be calculated correctly", 
    ...
    fun testAdaptiveRestCalculation_LogicAndReasoning() {
        val restCalc = AdaptiveRestCalculation(
            baseRestTime = 90f,
        ...
        val expectedAdjusted = restCalc.baseRestTime * restCalc.fatigueAdjustment * 
                              restCalc.performanceAdjustment * restCalc.intensityAdjustment
        assertEquals("Adjusted time should match calculation", 
                    expectedAdjusted, restCalc.adjustedRestTime, 2f)
    }
    ...
    fun testWorkoutSessionContext_ConfigurationData() {
        ...
        assertEquals("Should be 50% complete", 
                    0.5f, sessionContext.completedExercises.toFloat() / sessionContext.totalExercises, 0.01f)
        assertTrue("Intensity should be in valid range", 
                  sessionContext.overallIntensity >= 0f && sessionContext.overallIntensity <= 1f)
        assertTrue("Energy level should be in valid range", 
    ...
    fun testDataConsistency_RelatedModels() {
        ...
        val modification = AdaptiveExerciseModification(
            exerciseId = exercise.name,
            weightAdjustment = 0.1f,
            reason = "Excellent form"
        )
        
        val workoutAdaptation = WorkoutAdaptation(
        ...
        assertEquals("Exercise names should match", 
                    exercise.name, modification.exerciseId)
        assertTrue("Modification should be included in adaptation", 
                  workoutAdaptation.modifications.contains(modification))
        assertEquals("Adaptation type should match modification intent", 
                    AdaptationType.INCREASE_INTENSITY, workoutAdaptation.type)
    }
}