...
class HydrationGoalUseCaseReactiveTest {
    ...
    @Test
    fun `reactive flow emits initial value immediately`() =
        runTest {
    ...
    @Test
    fun `reactive flow updates when daily goal changes`() =
        runTest {
    ...
    @Test
    fun `reactive flow updates when user preferences change`() =
        runTest {
    ...
    @Test
    fun `reactive flow prioritizes daily goal over preferences when both change`() =
        runTest {
    ...
    @Test
    fun `reactive flow falls back to preferences when daily goal removed`() =
        runTest {
    ...
    @Test
    fun `reactive flow uses default when all sources are empty`() =
        runTest {
    ...
    @Test
    fun `todays reactive flow uses timezone-safe date calculation`() =
        runTest {
    ...
    @Test
    fun `reactive flow handles zero water goal in daily goal entity`() =
        runTest {
            // Given: Daily goal with zero water target
            val dailyGoalWithZeroWater =
                DailyGoalEntity(
                    dateIso = testDate.toString(),
                    targetKcal = 2000,
                    targetWaterMl = 0,
