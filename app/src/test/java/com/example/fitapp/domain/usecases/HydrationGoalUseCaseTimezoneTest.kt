...
class HydrationGoalUseCaseTimezoneTest {
    ...
    @Test
    fun `today's hydration goal uses system timezone`() = runTest {
    ...
    @Test
    fun `reactive flow updates when timezone changes daily goal availability`() = runTest {
    ...
    @Test
    fun `DST spring forward day boundary handling`() = runTest {
        ...
        val beforeGoal = DailyGoalEntity(
            dateIso = beforeDst.toString(),
            targetKcal = 2000,
            targetWaterMl = 2500
        )
        val dstGoal = DailyGoalEntity(
            dateIso = dstDate.toString(),
            targetKcal = 2000,
            targetWaterMl = 3000
        )
        val afterGoal = DailyGoalEntity(
    ...
    @Test
    fun `DST fall back day boundary handling`() = runTest {
    ...
    @Test
    fun `timezone offset changes don't affect LocalDate-based goals`() = runTest {
    ...
    @Test
    fun `reactive flow handles concurrent updates correctly`() = runTest {
        // Given: Initial setup
    ...
    @Test
    fun `year boundary dates handle correctly`() = runTest {
